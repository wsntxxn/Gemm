...
void matmul(datatype** a, datatype** b, datatype** c, int m, int n, int k) { 
  ...
  recursive_matmul(0, 0, 0, 0, 0, 0, m, n, k, a, b, c);
#else 
  printf("use the dumb method...\n");
  dumb_matmul(a, b, c, m, n, k);
#endif
}
...
int main (int argc, char * argv[]) 
{
  ...
  read_matrix(argv[1], &a, &sa, &m, &n);

  /* read matrix B */
  printf("read matrix B from %s\n", argv[2]);
  read_matrix(argv[2], &b, &sb, &n_tmp, &k);
  if(n != n_tmp) { printf("ERROR: matrix A and B incompatible\n"); return 2; }

  /* initialize matrix C */
  sc = (datatype*)malloc(m * k *sizeof(datatype));
  memset(sc, 0, m * k *sizeof(datatype));
  c = (datatype**)malloc(m * sizeof(datatype*));
  for(int i = 0; i < m; i++) c[i] = &sc[i * k];

  /* do the multiplication */
  matmul(a, b, c, m, n, k);
  
  /* write matrix C */
  write_matrix(argv[3], sc, m, k);
  /*
  for (i=0; i<n; i++) {
